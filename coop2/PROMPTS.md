...
| 4 | 当前合作模式规则 -- COOPERATION MODE (individual / broadcast_chain / centralized leader / centralized follower / decentralized_messageboard) | `prompt_sections.py` `COOPERATION_RULES`, `cooperation_section`; the *reserved* notify rule `MESSAGEBOARD_NOTIFY_RULES` is appended only while `MessageboardTeamBrain.NOTIFY_TOOL_ENABLED` | `TeamBrain.COOPERATION_MODE`, set per brain class in `llm_team.py` (`TeamBrain`, `ChainTeamBrain`, `LeaderTeamBrain`, `FollowerTeamBrain`, `MessageboardTeamBrain`) |
...
| Notify request (*reserved*; the notify tool) | `llm_client.py` `NotifyRequest`, `LLMMessageboardNotifyPlanResponse.notify`; recorded by `message_board.py` `MessageBoard.notify`, delivered only through `MessageBoard.deliverer` (none installed) | off until `MessageboardTeamBrain.NOTIFY_TOOL_ENABLED`; then the model may name teams to interrupt and what to tell them |
...
- [`feasibility_verify/test_messageboard_stubbed.py`](../feasibility_verify/test_messageboard_stubbed.py) -- the board mode: section 4, the board as section 7 with `(new)` and `You`, `board_post` appended in the planning call and read by the next team, the notify tool reserved behind its flag.
